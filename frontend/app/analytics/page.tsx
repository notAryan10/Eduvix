...
export default function AnalyticsPage() {
  ...
  if (!data) return (
    <DashboardLayout>
      <div className="p-20 text-center font-bold">No data found. Take a quiz to start!</div>
    </DashboardLayout>
  );
          ...
          <section className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
            ...
            <div className="space-y-4">
              {data.profile.weakTopics?.length > 0 ? (
                ...
                <p className="text-gray-500 italic">Keep studying to identify areas for improvement!</p>
              )}
            </div>
          </section>
            ...
            <div className="space-y-4">
              {data.recentQuizzes?.length > 0 ? (
                data.recentQuizzes.map((q: any, i: number) => (
                  <div key={i} className="flex justify-between items-center p-4 border-b last:border-0 border-gray-50">
                    <div>
